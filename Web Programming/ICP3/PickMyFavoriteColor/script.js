...
function addBox(hexadecimal, domObject) {
    addColor = document.createElement("div");
    addColor.setAttribute('class', 'favorite_color');
    addColor.setAttribute("style", `background-color:${hexadecimal}`);
    domObject.appendChild(addColor);
}
...
//3.Write events handlers such that whenever any item in the favorite colors is clicked or hovered, the color gets displayed in the preview div
    ...
    $(document).on('mouseover click', '.favorite_color', function(){
        updateColor = $(this).css("background-color");
        setPreviewColor(updateColor);
    });
});
